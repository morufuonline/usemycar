...
if($c != ""){ 
$c = testQty($c);
$act = $db->delete("users_messages", " id='{$c}' AND sender_email = '$user_email' AND sent = '1'");	
...
$result = $db->select("users_messages", "WHERE sender_email = '$user_email' AND sent = '1'", "*", "ORDER BY id DESC");
...
$result = $db->select("users_messages", "WHERE sender_email = '$user_email' AND sent = '1'", "*", "ORDER BY id DESC", "LIMIT {$offset},{$per_view}");

if(count_rows($result) > 0){
?>
<form action="sent-messages.php" method="post" runat="server" autocomplete="off" enctype="multipart/form-data" style="overflow-x:auto;">
...
//=======================View Post==============================//
if(!empty($view)){
$result = $db->select("users_messages", "WHERE id='$view' AND sender_email = '$user_email' AND sent = '1'", "*", "");
