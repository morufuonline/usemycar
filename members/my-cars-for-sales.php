...
$act = $db->query("UPDATE sellable_cars SET status = '2' WHERE id = '{$sold}' AND user_id = '{$id}'");
...
redirect("my-cars-for-sales.php?pn={$pn}");
}else{
...
$result = $db->select("sellable_cars", "WHERE user_id = '$id'", "*", "ORDER BY id DESC");

$per_view = 20;
$page_link = "my-cars-for-sales.php?pn=";
$link_suffix = "";
$style_class = "";
page_numbers();
...
$result = $db->select("sellable_cars", "WHERE user_id = '$id'", "*", "ORDER BY id DESC", "LIMIT {$offset},{$per_view}");
...
<td><?php if($sta == 1){ ?>
<a onClick="javascript: return confirm('Are you sure you have sold this car?')" href="my-cars-for-sales.php?sold=<?php echo $ad_id; ?>&pn=<?php echo $pn; ?>" class="btn gen-btn" title="Mark ad #<?php echo $ad_id; ?> as sold"><i class="fa fa-check" aria-hidden="true"></i> Mark</a>
<?php } ?></td>
<td><a href="my-cars-for-sales.php?view=<?php echo $ad_id; ?>&pn=<?php echo $pn; ?>" class="btn gen-btn" title="View ad #<?php echo $ad_id; ?>"><i class="fa fa-eye" aria-hidden="true"></i> View</a></td>
</tr>
<?php 
$d++;
}
...
echo "<div class='alert alert-danger alert-dismissable fade in'>No cars found at the moment.</div>";
}
...
//=======================View Post==============================//
if(!empty($view)){
$result = $db->select("sellable_cars", "WHERE id='$view' AND user_id = '$id'", "*", "");
...
<div><a href="my-cars-for-sales.php?pn=<?php echo $pn; ?>" class="btn gen-btn"><i class="fa fa-arrow-left"></i> Back to Car Sales Ads</a></div>
